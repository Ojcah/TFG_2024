...
def main():
    ...
    with torch.no_grad():
        for pwm, lengths, angle in test_loader:
            ...
            predictions_seq = np.zeros(lengths.item())

            #prediction = model(pwm,lengths)
         
            for i in range(lengths.item()):  # Iterate only over the actual length of the sequence
                pwm_sample = pwm[:, i]
                prediction = model.predict(pwm_sample)
                predictions_seq[i] = prediction.cpu().numpy().item()

            predictions.append(predictions_seq)
            ground_truths.append(angle.squeeze().cpu().numpy())
            pwm_values.append(pwm.squeeze().cpu().numpy())

            index += 1
            print(f"Testing sequence {index}: ({len(predictions_seq)})")
